...
import { formatLevel, formatTokenQuantity, toLocalDateInputValue, toLocalTimeInputValue, londonWallTimeToUtc } from "@/lib/utils";
import { updateScheduledSession } from "@/lib/actions/bookings";
...
export function EditScheduledSessionForm({
  ...
  function handleSubmit(e: FormEvent) {
    ...
    startTransition(async () => {
      const result = await updateScheduledSession(bookingId, {
        subject,
    ...
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-5" noValidate>
      ...
      {success && (
        <p className="rounded-md bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
          Session updated — your client has been emailed.
        </p>
      )}
      ...
      <p className="text-xs text-navy/40">
        ...
        reserved tokens automatically. Your client is emailed about the
        change. {formatTokenQuantity(durationMinutes / 60)} token
